...
def time_to_next_birthday(b):
    ...
    print(
        "your age: ",
        time.year - birth.year - ((time.month, time.day) < (birth.month, birth.day)),
    )
    print(
        "time to next birthday: ",
        (datetime.datetime(next_bday[0], next_bday[1], next_bday[2]) - time).days,
        "days ;)",
    )
...
def find_older(b1, b2):
    return (
        (b1, b2)
        if (
            datetime.datetime(b1.year, b1.month, b1.day)
            - datetime.datetime(b2.year, b2.month, b2.day)
        ).days
        < 0
        else (b2, b1)
    )
...
def find_double_day(b1, b2):
    ...
    x = (
        datetime.datetime(st[1].year, st[1].month, st[1].day)
        - datetime.datetime(st[0].year, st[0].month, st[0].day)
    ).days
    y = 0
    ...
    print(
        "Older guy was ",
        (date - datetime.datetime(st[0].year, st[0].month, st[0].day)).days,
        " days old, and younger dude was ",
        (date - datetime.datetime(st[1].year, st[1].month, st[1].day)).days,
        " days old",
    )
